...
for i in range(len(a)):
    ...
    pic_link_list = page_with_pic_soup.find_all('a', class_ = 'galleryfilename galleryfilename-truncate')
    
    for tg in range(len(pic_link_list)):

        final_pic_page = pic_link_list[tg].get('href')
        final_pic_page_res = requests.get('https://commons.wikimedia.org' + final_pic_page)
        final_pic_page_soup = BeautifulSoup(final_pic_page_res.text, 'html.parser')
        final_img = final_pic_page_soup.find('img', decoding = "async")
        src = final_img.get('src')
        alt = final_img.get('alt')

        requests.get(src, headers = {'User-agent': 'your bot 0.1'})

        try:
            urllib.request.urlretrieve(src, "C:/Users/Admin/Desktop/Stamps of Soviet Union/%s/" % (str(1923 + i) ) + alt[5:])
        except OSError:
            print('Name Error')
